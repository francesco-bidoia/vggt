...
def rename_colmap_recons_and_rescale_camera(
    ...
    """Rescale cameras and points to match the original image resolution."""
    ...
    for pyimageid in reconstruction.images:
        ...
        if rescale_camera:
            ...
            x1, y1, x2, y2, width, height = original_coords[pyimageid - 1]

            max_dim = max(width, height)
            left = (max_dim - width) / 2
            top = (max_dim - height) / 2
            scale_factor = max_dim / img_size

            # scale focal lengths
            pred_params[0] *= scale_factor
            if len(pred_params) > 3:
                pred_params[1] *= scale_factor

            # shift principal point
            pred_params[-2] = pred_params[-2] * scale_factor - left
            pred_params[-1] = pred_params[-1] * scale_factor - top

            pycamera.params = pred_params
            pycamera.width = width
            pycamera.height = height

        if shift_point2d_to_original_res:
            x1, y1, _, _, width, height = original_coords[pyimageid - 1]
            ...
            for point2D in pyimage.points2D:
                point2D.xy = point2D.xy * scale_factor - np.array([left, top])
